...
public class DrawingView extends View
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        { // ALEX
            canvas.drawBitmap(canvasBitmap, 0, 0, canvasPaint);//Refresca la pantalla
            canvas.drawRect(calculateDirection(posx[1],
                                                posy[1],
                                                posx[0],
                                                posy[0]),
                                                drawPaint);
        ...
        {
            canvas.drawBitmap(canvasBitmap, 0, 0, canvasPaint);//Refresca la pantalla
            canvas.drawOval(calculateDirection(posx[1],
                                                posy[1],
                                                posx[0],
                                                posy[0]),
                                                drawPaint);
    ...
    private void pintarOvalo(MotionEvent event,boolean filled)
    {
        ...
        switch (event.getAction())
        {
            ...
            case MotionEvent.ACTION_UP:
                ...
                drawCanvas.drawOval(new RectF(posx[0],
                                                posy[0],
                                                posx[1],
                                                posy[1]),
                                                drawPaint);
    ...
    private float calcularBrushSize(JSONObject stream) throws JSONException
    {
        ...
        if(pixelesEnviador > pixeles)
            return reglaDeTres(pixelesEnviador,
                                pixeles,
                                (float)stream.getDouble("brushSize"));
        else
    ...
    private void setDrawText(JSONObject obj) throws JSONException
    {
        ...
        textX = reglaDeTres(obj.getInt("width"),
                            drawCanvas.getWidth(),
                            (float) obj.getDouble("tx"));

        textY = reglaDeTres(obj.getInt("height"),
                                        drawCanvas.getHeight(),
                                        (float) obj.getDouble("ty"));
    }
