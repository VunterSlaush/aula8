                ...
                public void onItemClick(AdapterView<?> adapterView, final View view, int i, long l) {
                    ...
                    Thread t = new Thread(new Runnable()
                    {
                        ...
                        public void run()
                        {
                            try
                            {
                                System.out.println("Direccion Descarga " + dir+file);
                                FilesController.donwloadFile(Utils.reemplazarSlash(dir+"/"+file), view.getContext());
                                while (!FilesController.getDownloadState());
                                Toast.makeText(view.getContext(), "Archivo Descargado! \n ver en "+Utils.getA8Folder(), Toast.LENGTH_SHORT).show();
